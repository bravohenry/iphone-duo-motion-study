...
- The download control renders the current camera, pose and screen content into a separate multisampled RGBA target, then exports a transparent PNG without the white stage or DOM controls. Its readback path flips WebGL rows and restores partially covered edge colors to avoid dark transparency fringes.
