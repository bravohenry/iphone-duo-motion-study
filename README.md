...
- The download control renders the current camera, pose and screen content into a separate multisampled RGBA target at up to 1.5× the output dimensions, then high-quality downsamples a transparent PNG without the white stage or DOM controls. Its 14-megapixel render budget, WebGL row flip and partial-alpha color recovery improve diagonal metal edges without dark transparency fringes or unbounded memory use.
