...
| Material passes | Lotus assigns six environment layers and separate scene-level opacity/exposure chunks. | Three.js restores the delivered layer 4/5 Optics/Finish EXRs, source rotations, Logo visibility and camera-glass transparency; layers 0–2 share the Finish fallback with restrained diffuse fill. | Fine highlight travel can still differ from Lotus's dynamic light-card interpolation, but black trim, Logo and optics no longer fall back to bare-glTF rendering. |
