...
The glTF is not a self-contained final material result: its Apple Logo highlight layer starts with zero Alpha and several camera-glass layers depend on Lotus scene-level transparency plus material-specific environment layers. The local adapter restores the Logo layer, preserves the optical stack, assigns the supplied Optics and Finish EXRs with their authored rotations, and replaces the earlier high-intensity three-light rig with restrained diffuse compensation.
